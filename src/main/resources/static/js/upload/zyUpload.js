/* www.jq22.com */
(function ($, undefined) {
    var SupportFormat = ["jpg", "png", "gif", "jpeg"];
    var MAINHOST = "http://192.168.0.113:8080";
    ...
    $.fn.zyUpload = function (options, param) {
        ...
        return this.each(function () {
            ...
            var defaults = {
                width: "700px",  					    // 宽度
                height: "400px",  					    // 宽度
                itemWidth: "149px",                     // 文件项的宽度
                itemHeight: "120px",                    // 文件项的高度
                url: MAINHOST + "/FileUpload/imgUpload",// 上传文件的路径
            ...
            this.createHtml = function () {
                ...
                if (para.dragDrop) {
                    ...
                    html += '						<div class="filePicker">点击选择工厂平面图片</div>';
            ...
            this.funFilterEligibleFile = function (files) {
                ...
                for (var i = 0, file; file = files[i]; i++) {
                    if (file.size >= 51200000) {
                        ...
                        var format;
                        if (index != -1)
                            format = file.name.substring(index + ".".length);

                        var isSupport = false;
                        //判断如果是图片文件则加入数组当中 否则提示不支持格式
                        $.each(SupportFormat, function (index, val) {
                            if (val.toLowerCase() == format.toLowerCase()) {
                                // 在这里需要判断当前所有文件中
                                arrFiles.push(file);
                                isSupport = true;
                            }
            ...
            this.funDisposePreviewHtml = function (file, e) {
                ...
                var delHtml = "";
                if (para.del) {  // 显示删除按钮
                    delHtml = '<div class="file_del" data-index="' + file.index + '" title="删除"></div>';
                }

                // 处理不同类型文件代表的图标
                var fileImgSrc = "css/upload/image/fileType/";
            ...
            this.addEvent = function () {
                ...
                $(".upload_btn").bind("click", function (e) {
                    $.ajax({
                        url: MAINHOST + "/FileUpload/getAllFactorys",
                        type: "GET",
                        dataType: "json",
                        success: function (result) {
                            if (result.code == 200) {
                                var factorys = result.data;

                                $.each(factorys, function (index, obj) {
                                    var node = "<option value='" + index + 1 + "'>" + obj.FactoryName + "</option>";
                                    $(".selectpicker").append($(node));
                                });
